...
"""This class contains unit tests for the entire application"""
class BookAMealTestCase(unittest.TestCase):
    ...
    def test_api_get_all_meals(self):
    ...
    def test_api_get_meal_by_id(self):
        ...
        self.assertEqual(res.status_code, 201)
        result_in_json = json.loads(res.data.decode('utf-8').replace("'", "\""))
        result = self.client.get('/meals/{}'.format(result_in_json['id']))
        self.assertEqual(result.status_code, 200)
        self.assertIn('Posho & Meat', str(res.data))

    def test_api_edit_order(self):
