...
class AuthTestCase(unittest.TestCase):
    ...
    def test_user_registered(self):
        ...
        res = self.client().post('/auth/register', data=test_data)
    ...
    def test_user_already_registered(self):
        ...
        res = self.client().post('/auth/register', data=test_data)
        es = self.client().post('/auth/register', data=test_data)
        self.assertEqual(res.status_code, 409)
        second_res = self.client().post('/auth/register', data=self.user)
