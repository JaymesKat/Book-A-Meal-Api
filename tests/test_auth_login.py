...
class LoginTestCase(unittest.TestCase):   
    ...
    def test_registered_user_log_in(self):
        ...
        res = self.client().post('/bookameal/api/v1/auth/register/', data=json.dumps(test_data))
        registered_data = json.loads(res.data.decode())
        self.assertTrue(
            registered_data['message'] == 'User {} was created'.format(test_data['user_name'])
        )
        self.assertEqual(res.status_code, 201)

        res1 = self.client().post('/bookameal/api/v1/auth/login/', data=json.dumps({"email": "joshua@example.com", "password": "@password"}),content_type='application/json')
    ...
    def test_user_logging_in_with_missing_credentials(self):
        ...
        res = self.client().post('/bookameal/api/v1/auth/login/', data=test_data, content_type='application/json')
