...
class LoginTestCase(BaseTest):   
    ...
    def test_registered_user_log_in(self):
        ...
        res = self.client.post('/api/v1/auth/register/', data=json.dumps(test_data))
        ...
        res1 = self.client.post('/api/v1/auth/login/', data=json.dumps({"email": "joshua@example.com", "password": "@password"}),content_type='application/json')
    ...
    def test_user_logging_in_with_missing_credentials(self):
        ...
        res = self.client.post('/api/v1/auth/login/', data=test_data, content_type='application/json')
